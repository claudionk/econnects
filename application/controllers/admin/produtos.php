...
class Produtos extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('produto_venda_model', 'produto_venda');
    }
    ...
    public function index($offset = 0) //Função padrão (load)
    {
        ...
        $config['total_rows'] =  $this->current_model->filterFromInput()->get_total();
        $config['per_page'] = 20;
        ...
        $data = array();
        $data['rows'] = $this->current_model->filterFromInput()->order_by('produto.nome')->limit($config['per_page'], $offset)->with_produto_ramo()->get_all();


        $data['primary_key'] = $this->current_model->primary_key();
        $data["pagination_links"] = $this->pagination->create_links();
        $data['ramos'] = $this->produto_ramo->get_all();
        $data['venda'] = $this->produto_venda->get_all();
    ...
    public function add() //Função que adiciona registro
    {
        ...
        $this->template->set('page_title_info', '');
        $this->template->set('page_subtitle', "Adicionar Produto");
    ...
    public function edit($id) //Função que edita registro
    {
        ...
        //Setar variáveis de informação da página
        $this->template->set('page_title_info', '');
        $this->template->set('page_subtitle', "Editar Produto");
