 ...
 *
 */
class Comissao extends Admin_Controller
{
    public function __construct() 
    {
        ...
        $this->template->set('page_title', "Comissão");
        $this->template->set_breadcrumb("Tipo de Comissão", base_url("$this->controller_uri/index"));

        //Carrega modelos
        $this->load->model('comissao_model', 'current_model');
    ...
    public function index($offset = 0) //Função padrão (load)
    {
        ...
        $this->template->set_breadcrumb("Comissão", base_url("$this->controller_uri/index"));
        ...
        $data = array();
        $data['rows'] = $this->current_model->with_foreign()->limit($config['per_page'], $offset)->get_all();
    ...
    public function add() //Função que adiciona registro
    {
        ...
        $this->template->js(app_assets_url('modulos/comissao/base.js', 'admin'));

        //Setar variáveis de informação da página
        $this->template->set('page_title_info', '');
        $this->template->set('page_subtitle', "Adicionar Tipo de Comissão");
        ...
        $data['tipos'] = $this->comissao_tipo->get_all();
    ...
    public function edit($id) //Função que edita registro
    {
        ...
        //Setar variáveis de informação da página
        $this->template->set('page_title_info', '');
        $this->template->set('page_subtitle', "Editar Tipo de Comissão");
