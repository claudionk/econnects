 ...
 *
 */
class Fatura extends Admin_Controller
{
    public function __construct() 
    {
        ...
        $this->template->set('page_title', "Faturas");
        $this->template->set_breadcrumb("Faturas", base_url("$this->controller_uri/index"));

        //Carrega modelos
        $this->load->model('fatura_model', 'current_model');
    ...
    public function index($offset = 0) //Função padrão (load)
    {
        ...
        $this->template->set('page_subtitle', "Faturas");
        $this->template->set_breadcrumb("Faturas", base_url("$this->controller_uri/index"));
        ...
        $config['total_rows'] =  $this->current_model->with_fatura_status()->with_pedido()->get_total();
        $config['per_page'] = 10;
        ...
        $data = array();
        $data['rows'] = $this->current_model->limit($config['per_page'], $offset)->with_fatura_status()->with_pedido()->get_all();
        $data['primary_key'] = $this->current_model->primary_key();
        $data["pagination_links"] = $this->pagination->create_links();

        foreach ($data['rows'] as $index => $row) {
            $data['rows'][$index]['parcelas'] = $this->parcela->with_fatura_status()
                                                              ->filterByFatura($row['fatura_id'])
