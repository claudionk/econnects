...
class Venda_Seguro_Saude extends Admin_Controller {
  ...
  public function calculo() {
    ...
    $valores_bruto = $this->getValoresPlano( $produto_parceiro_id, $quantidade, $servico_produto_id, $cotacao["data_nascimento"] );
  ...
  /**
     ...
     * @return array
     */
  private function getValoresPlano($produto_parceiro_id, $quantidade = 1, $servico_produto_id = 0, $data_nascimento ){
    ...
    foreach ($arrPlanos as $plano){
      switch ((int)$plano['precificacao_tipo_id'])
      {
        case self::PRECO_TIPO_TABELA:
          ...
          $idade = (date("md", date("U", mktime(0, 0, 0, $birthDate[2], $birthDate[1], $birthDate[0]))) > date("md")
  ...
  private function getValorTabelaFixa( $produto_parceiro_plano_id, $idade ){
    ...
    $valor = $this->itens->filter_by_produto_parceiro_plano($produto_parceiro_plano_id)
      ->filter_by_faixa_etaria( intval( $idade ) )
