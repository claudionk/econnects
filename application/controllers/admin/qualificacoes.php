 ...
 * Class Qualificacoes
 *
 * @property Qualificacao_Model $current_model
 *
 */
class Qualificacoes extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->template->set('page_title', "Qualificações");
        $this->template->set_breadcrumb("Qualificações", base_url("$this->controller_uri/index"));

        //Carrega modelos
        $this->load->model('qualificacao_model', 'current_model');
    ...
    public function index($offset = 0) //Função padrão (load)
    {
        ...
        $this->template->set('page_subtitle', "Qualificações");
        $this->template->set_breadcrumb("Qualificações", base_url("$this->controller_uri/index"));
        ...
        $data = array();
        $data['rows'] = $this->current_model->limit($config['per_page'], $offset)->with_produto()->get_all();
    ...
    public function add() //Função que adiciona registro
    {
        ...
        $this->template->set('page_title_info', '');
        $this->template->set('page_subtitle', "Adicionar Qualificação");
    ...
    public function edit($id) //Função que edita registro
    {
        ...
        //Setar variáveis de informação da página
        $this->template->set('page_title_info', '');
        $this->template->set('page_subtitle', "Editar Qualificação");
    ...
    public function ranking($id){
        ...
        $data = array();
        $rows = $this->current_model->coreRanking()
            ->coreRakingFilterByQualificacao($id)
            ->coreRakingPontuacaoTotalByParceiro()
            ->get_all();
        ...
        //Carrega template
        $this->template->load("admin/layouts/base", "$this->controller_uri/ranking", $data );
