 ...
 *
 */
class Capitalizacao extends Admin_Controller
{
    public function __construct()
    {
        ...
        $this->template->set('page_title', 'Capitalização');
        $this->template->set_breadcrumb('Capitalização', base_url("$this->controller_uri/index"));
        
        //Carrega modelos necessários
        $this->load->model('capitalizacao_model', 'current_model');
    }
    
    public function index($offset = 0)
    {
        //Carrega models necessários
        $this->load->model('capitalizacao_tipo_model', 'tipo');
        ...
        $data['rows'] = $this->current_model->limit($config['per_page'], $offset)->with_capitalizacao_tipo()->get_all();
    ...
    public function add()
    {
        //Carrega models necessários
        $this->load->model('capitalizacao_tipo_model', 'tipo');
        $this->load->model('capitalizacao_sorteio_model', 'sorteio');
        ...
        $this->template->set('page_title_info', '');
        $this->template->set('page_subtitle', 'Adicionar Capitalização');
        ...
        $data['sorteio'] = $this->sorteio->get_all();
