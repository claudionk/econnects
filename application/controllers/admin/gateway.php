...
class Gateway extends Admin_Controller
{
    ...
    foreach ($pedidos as $index => $pedido) {
      ...
      try {
        ...
        if (count($cartao) > 0) {
          ...
          $transacao = $this->transacao->filter_by_cartao($cartao['pedido_cartao_id'])
            ->get_all();
          ...
          if ($cartao['erros'] >= $integracao['qnt_erros']) {
  ...
  public function pagmax_efetuar_pagamento($pedido_cartao_id) {
    ...
    try
    {
      ...
      $isdebit = ($parceiro_pagamento['produto_parceiro_pagamento_id'] == 6 ) ? TRUE : FALSE; //ReturnURL
      ...
      error_log( print_r( $JsonDataRequest, true ) . "\n", 3, "/var/log/nginx/360.log" );
