 ...
 *
 */
class Localidade_Bairros extends Admin_Controller
{
    public function __construct()
    {
        ...
        $this->template->set('page_title', "Bairros");
        $this->template->set_breadcrumb("Bairros", base_url("$this->controller_uri/index"));

        //Carrega modelos
        $this->load->model('localidade_bairro_model', 'current_model');
        $this->load->model('localidade_cidade_model', 'cidades');
        $this->load->model('localidade_estado_model', 'estados');
    }
    
    public function buscaBairrosPorCidades()
    {   
        ...
        $data = $this->current_model->getBairrosPorCidades($this->input->post("id_cidade"));
    ...
    public function index($offset =0)
    {
        ...
        $this->template->set('page_subtitle', "Bairros");
        $this->template->set_breadcrumb("Bairros", base_url("$this->controller_uri/index"));
    ...
    public function add()
    {
        ...
        $this->template->js(app_assets_url('modulos/localidade_bairros/base.js', 'admin'));
        $this->template->js(app_assets_url('modulos/localidade_cidades/base.js', 'admin'));

        //Setar variáveis de informação da página
        $this->template->set('page_title_info', '');
        $this->template->set('page_subtitle', "Adicionar Bairro");
    ...
    public function edit($id)
    {
        ...
        //Carrega dados para a página
        $data = array();
        $data['row'] = $this->current_model->with_cidade_estado()->get($id);
        $data['primary_key'] = $this->current_model->primary_key();
        $data['new_record'] = '0';
        $data['form_action'] =  base_url("$this->controller_uri/edit/{$id}");
       // $data['cidades'] = $this->cidades->get_all(); //Resgata todas os estados
        $data['estados'] = $this->estados->get_all();
