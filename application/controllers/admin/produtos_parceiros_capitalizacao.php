 ...
 *
 */
class Produtos_Parceiros_Capitalizacao extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->template->set('page_title', "Produtos / Parceiro / Capitalização");
        $this->template->set_breadcrumb("Produtos / Parceiro / Capitalização", base_url("$this->controller_uri/index"));

        //Carrega modelos
        $this->load->model('produto_parceiro_capitalizacao_model', 'current_model');
    ...
    public function index($produto_parceiro_id , $offset = 0)
    {
        ...
        $this->auth->check_permission('view', 'produto_parceiro_capitalizacao', 'admin/parceiros/');
        ...
        $this->template->set('page_subtitle', "Capitalização");
        $this->template->set_breadcrumb("Capitalização", base_url("$this->controller_uri/view_by_produto_parceiro/{$produto_parceiro_id}"));
    ...
    public function add($produto_parceiro_id){
        ...
        $this->template->set('page_title_info', '');
        $this->template->set('page_subtitle', "Capitalização");
        $this->template->set_breadcrumb("Capitalização", base_url("$this->controller_uri/index"));
        ...
        $data['produto_parceiro_id'] = $produto_parceiro_id;
        $data['produto_parceiro'] = $produto_parceiro;
        $data['capitalizacao'] = $this->capitalizacao->with_capitalizacao_tipo()->get_all();
    ...
    public function edit($id) //Função que edita registro
    {
        ...
        $this->template->set('page_title_info', '');
        $this->template->set('page_subtitle', "Editar Capitalização");
        $this->template->set_breadcrumb('Capitalização', base_url("$this->controller_uri/index"));
