...
class Campos extends CI_Controller {
  ...
  public function index() {
    ...
    if( !isset( $GET["produto_parceiro_plano_id"] ) ) {
      ...
      $planos = $this->produto_parceiro_plano->coreSelectPlanosProdutoParceiro( null, $produto_parceiro_plano_id )->get_all_select();
      $result = array();
      ...
      if( $planos ) {
        ...
        foreach ($campos_tipo as $index => $item) {
          $campos = $this->produto_parceiro_campo->coreSelecCampoProdutoParceiro( $produto_parceiro_plano_id, $item["campo_tipo_id"] )->get_all_select();
          if( $campos ) {
      ...
      if( $result ) {
        ...
        die( json_encode( array( "status" => false, "message" => "Não foram localizados os campos do produto_parceiro_plano_id $produto_parceiro_plano_id" ) ) );
      }
