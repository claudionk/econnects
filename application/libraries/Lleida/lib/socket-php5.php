...
if(!class_exists('Socket')){
    define('VSMS_SOCKET5_VERSION', '1.2');
    ...
    class Socket5 {
        ...
        function connect($connData = array()){
            ...
            $this->con = stream_socket_client("tcp://".$host.":".$port, $errno, $errstr, SOCKET_TIMEOUT);
            //socket_create(AF_INET, SOCK_STREAM, SOL_TCP);

            if (!$this->con) {
                if(SOCKET_DEBUG) $this->log->debug('[ Socket5  ] '.$this->objID.' Error '.$errstr.' ('.$errno.')');
                $this->estat = SOCKET_NOCONNECT;
