...
class Pagmax360 {
  ...
  //
  public function getCardToken( $merchantId, $merchantKey, $Json, $Env = "teste" ) {
    ...
    //$logPagmax = $this->CI->db->query( "call sp_loga_transacao_pagmax( $merchantId, '$Url', '$Json', '$Response', " . substr( $JsonObject->{"MerchantOrderId"}, 3 ) . " )" );
  ...
  public function voidTransaction( $merchantId, $merchantKey, $PaymentID, $Env = "teste" ) {
    ...
    if( isset( $JsonObject->{"MerchantOrderId"} ) ) {
	    //$logPagmax = $this->CI->db->query( "call sp_loga_transacao_pagmax( $merchantId, '$Url', '', '$Response', " . substr( $JsonObject->{"MerchantOrderId"}, 3 ) . " )" );
    }
  ...
  public function getTransactionByMerchantOrderId( $merchantId, $merchantKey, $MerchantOrderId, $Env = "teste" ) {
    ...
    if( isset( $JsonObject->{"Payments"} ) ) {
	    //$logPagmax = $this->CI->db->query( "call sp_loga_transacao_pagmax( $merchantId, '$Url', '', '$Response', " . substr( $MerchantOrderId, 3 ) . " )" );
    }
    return $Response;
  }

  function returnTransactionStatusCode( $statusCode ) {
    $statusCodes = array(
