...
Class Base_Pessoa_Model extends MY_Model
{
    ...
    function getByDoc($documento, $produto_parceiro_id){
        ...
        if($result){
           ...
           if($result['atualizado'] == 0){
               //faz a atualização do cliente.
                ...
                $this->updateCliente($documento,  $result['base_pessoa_id'], $produto_parceiro_id);
                $result = $this->_database->query($sql)->result_array();
                $result = $result[0];
            ...
            $this->updateCliente($documento,  0, $produto_parceiro_id);
            $result = $this->_database->query($sql)->result_array();
            if($result) {
                $result = $result[0];
                $result['contato'] = $this->base_pessoa_contato->with_contato_tipo()->filter_by_base_pessoa($result['base_pessoa_id'])->order_by('ranking')->get_all();
    ...
    public function updateCliente($documento,  $base_pessoa_id = 0, $produto_parceiro_id){
    ...
    function update_base_pessoa($base_pessoa_id, $data){
