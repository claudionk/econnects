...
class Apolice_Model extends MY_Model
{
     ...
     * @since  04/09/2019 
     */
    public function emissaoParcela($POST = []) {

        if( empty( $POST["apolices"] ) ) {
            return array( "status" => false, "message" => "Campo apolices é obrigatório" );
        }
        ...
        foreach ($POST["apolices"] as $key => $value) {
            ...
            if ( $this->getApoliceStatus($apolice_id, 'cancelada') ) {
        ...
        return array("status" => true, "message" => "Inserido com sucesso", "apolices" => $result);
    }
    ...
    public function validarDadosEntrada($POST = [])
    {
        ...
        if (empty($POST)) {
            return array( "status" => false, "message" => "Dados não enviados" );
        }
        ...
        if( isset( $POST["apolice_id"] ) ) {
            ...
            return array( "status" => false, "message" => "Campo apolice_id é obrigatório" );
        }
        ...
        if(!$pedido) {
            return array( "status" => false, "message" => "Apólice não encontrada" );
        }
