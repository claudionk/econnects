...
Class Comissao_Gerada_Model extends MY_Model
{
    ...
    public function gerar_comissao_parceiro_relacionamento(){
        ...
        $sql = "
                ...
                AND cotacao.deletado = 0
                AND pedido.deletado = 0		
                AND pedido.pedido_id NOT IN (SELECT comissao_gerada.pedido_id FROM comissao_gerada WHERE comissao_gerada.deletado = 0 AND comissao_gerada.comissao_classe_id = 4 )
              --  AND pedido.pedido_id = 2 -- @todo retirar
        ...
        foreach ($result as $item) {
            ...
            if($parceiro_relacionamento){
                $parceiro_relacionamento = $parceiro_relacionamento[0];

                $pai_id = $parceiro_relacionamento['pai_id'];

                while ($pai_id != 0){
                    $relacionamentos = $this->parceiro_relacionamento_produto->get($pai_id);

                    if($relacionamentos){
                        $comissao_venda =  ($relacionamentos['comissao']/100) * $premio_liquido_total;
                        ...
                        $data_comissao['pedido_id'] = $item['pedido_id'];
                        $data_comissao['valor'] = $comissao_venda;
                        $data_comissao['parceiro_id'] = $relacionamentos['parceiro_id'];
                        $data_comissao['premio_liquido_total'] = $premio_liquido_total;
                        $data_comissao['comissao'] = $relacionamentos['comissao'];

                        $data_comissao['descricao'] = "COMISSÃO {$comissao_classe['nome']} (". app_format_currency($relacionamentos['comissao'], false, 3) ."%) REFERENTE AO PEDIDO {$item['codigo']}";
                        $data_comissao['faturado'] = 0;

                        $this->insert($data_comissao, TRUE);
                    }

                    $pai_id = $relacionamentos['pai_id'];
