    ...
    <div class="form-group">
        ...
        foreach($list[$field_name] as $linha) { ?>
            <div class="col-md-<?php echo $tamanho ?> form-group<?php echo (app_is_form_error($field_name)) ? ' has-error' : ''; ?>">
             ...
             <input class="form-control" type="text" <?php echo issetor($class) ?> id="qtd_<?php echo (int)$linha['inicial']; ?>" name="qtd[<?php echo (int)$linha['inicial']; ?>]" type="text" value="<?php echo isset($linha['qtd']) ? (int)$linha['qtd'] : 0; ?>" /> 
             </div>     
        <?php }  ?>
