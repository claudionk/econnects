$(function () {
  ...
  $(".trocarParceiro").click(function () {
    $.ajax({
      type: "POST",
      url:
        base_url + "admin/colaboradores/trocarParceiro/" + $(this).attr("id"),
      dataType: "json",
  ...
  $(".datepicker_vendor_end").datepicker({
    format: "dd/mm/yyyy",
    language: "pt-BR",
    maxDate: 5,
    endDate: new Date(),
  ...
  $(".datepicker_vendor_start").datepicker({
    format: "dd/mm/yyyy",
    language: "pt-BR",
    maxDate: 5,
    endDate: new Date(),
  ...
  $(".validateEmail").on("blur", function (e) {
    ...
    var email = $.trim($(el).val()),
      parse_email = /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/;
  ...
  $(".inputmask-moeda").inputmask("999,99", {
    numericInput: true,
    rightAlignNumerics: false,
    greedy: true,
  });
  $(".inputmask-moeda2").inputmask("999.999.999,99", {
    numericInput: true,
    rightAlignNumerics: false,
    greedy: true,
  });
  $(".inputmask-valor").maskMoney({
    symbol: "R$ ",
    thousands: ".",
    decimal: ",",
    symbolStay: true,
  ...
  $("#validateSubmitForm")
    .find("input[type=text],input[type=tel],textarea,select")
    .first()
    .focus();
        ...
        return {
          ...
          categoria_pai_id:
            $(".js-categorias-ajax").val() != ""
              ...
              : 0,
          marca_id:
            $(".js-equipamento_marca_id-ajax").val() != ""
              ? $(".js-equipamento_marca_id-ajax").val()
              : 0,
        };
      },
  ...
  if ($(".js-equipamento_sub_categoria_id-ajax").data("selected") != "") {
    populaSelectSubCategoria(
      $(".js-equipamento_sub_categoria_id-ajax").data("selected")
    );
  }
        ...
        return {
          ...
          categoria_id:
            $(".js-categorias-ajax").val() != ""
              ...
              : 0,
          sub_categoria_id:
            $(".js-equipamento_sub_categoria_id-ajax").val() != ""
              ? $(".js-equipamento_sub_categoria_id-ajax").val()
              : 0,
          marca_id:
            $(".js-equipamento_marca_id-ajax").val() != ""
...
function busca_cliente() {
      ...
      if (result.cliente_id > 0) {
        ...
        $(".ls-modal").on("click", function (e) {
          ...
          $("#detalhe-cliente")
            .modal("show")
            .find(".modal-body")
            .load($(this).attr("href") + "/" + result.cliente_id);
        });
      } else {
  ...
  // Busca no SGS
  var url =
    "https://sgs-h.jelastic.saveincloud.net/v1/api/segurado/econnects?seg_doc=" +
    $("#cnpj_cpf").val();
  console.log(url);
  $.ajax({
    type: "GET",
...
function buscaDadosEAN() {
  ...
  if (typeof ean == "undefined" || ean == null || ean == "") return false;
...
function formatRepoEquipamento(repo) {
  ...
  var markup =
    ...
    "<div class='select2-result-repository__meta'>" +
    "<div class='select2-result-repository__title'>Equipamento: " +
    repo.nome +
    "<br/><span class='text-bold'>EAN:" +
    repo.ean +
    "</span></div>";
  ...
  markup +=
    ...
    "<div class='select2-result-repository__forks'><i class='fa fa-map-marker'></i> <span class='text-bold'>MARCA: </span>" +
    repo.equipamento_marca_nome +
    "</div>" +
    "<div class='select2-result-repository__forks'><i class='fa fa-folder-open'></i> <span class='text-bold'>LINHA / CATEGORIA: </span>" +
    repo.equipamento_categoria_nome +
    " - " +
    repo.equipamento_categoria_codigo +
    "</div>" +
    "<div class='select2-result-repository__forks'><i class='fa fa-folder-open'></i> <span class='text-tags'>TAGS: </span>" +
    repo.tags +
    "</div>" +
    "</div>" +
    //        "<div class='select2-result-repository__statistics'>" +
