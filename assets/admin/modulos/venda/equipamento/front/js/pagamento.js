  ...
  $.ajax({
    ...
    success: function (json) {
      if (json.success) {
        ...
        $("#sacado_endereco").val(
          json.data.tipo_logradouro.toUpperCase() +
            " " +
            json.data.logradouro.toUpperCase()
        );
...
$(function () {
  ...
  $(".numeros_cartao").inputmask("9999 9999 9999 9999", {
    placeholder: "____ ____ __executaPagamento__ ____",
  });
});

function executePagamento(pedido_id) {
  console.log("front executePagamento");
  $.ajax({
    type: "POST",
    url: base_url + "admin/gateway/executaPagamento/" + pedido_id,
    cache: false,
  }).done(function (result) {
    setTimeout(getStatusPedido, 2000);
  });
